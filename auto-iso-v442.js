(function(){
  ...
  function sharpHandheldLimit(f,ci){
    ...
    // Balanced, Sharp and Frozen keep the conservative sharp-handheld floor.
    // Intentional blur progressively permits slower speeds.
    if(ci.freeze<0.5){
      safe*=Math.pow(2,(0.5-ci.freeze)*6);
    }
    return safe;
  }
  ...
  function enhanceWhy(){
    ...
    if(!host)return;
    host.querySelectorAll('[data-v442-note],[data-v440-note]').forEach(function(n){n.remove();});
    var ul=host.querySelector('ul');
    if(!ul)return;
    if(!isFilm()&&autoOn&&autoCtx){
      var li=document.createElement('li');
      li.dataset.v442Note='1';
      ...
      if(autoCtx.clipped){
        var lim=document.createElement('li');lim.dataset.v442Note='1';
    ...
    if(isFilm()){
      ...
      if(f){
        var g=document.createElement('li');g.dataset.v442Note='1';
  ...
  function styles(){
    if(document.getElementById('v442AutoISOStyle'))return;
    var s=document.createElement('style');s.id='v442AutoISOStyle';
